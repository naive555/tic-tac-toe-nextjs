...
const makeRequest = (body: object) =>
  new NextRequest('http://localhost/api/game/play', {
    method: 'POST',
    body: JSON.stringify(body),
  });

describe('POST /api/game/play', async () => {
  const { POST } = await import('./route');

  it('returns 401 when no session', async () => {
    mockGetSession.mockResolvedValueOnce(null);

    const res = await POST(makeRequest({ board: [], position: 0 }));
    expect(res.status).toBe(401);
    expect(await res.json()).toEqual({ message: 'Unauthorized' });
  });

  it('returns game result on success', async () => {
    mockGetSession.mockResolvedValueOnce(mockSession as never);
    mockApi.post.mockResolvedValueOnce({ data: { board: [], result: 'WIN' } });

    const res = await POST(makeRequest({ board: [], position: 0 }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ board: [], result: 'WIN' });
    expect(mockApi.post).toHaveBeenCalledWith('/game/play', {
      board: [],
      position: 0,
    });
  });
