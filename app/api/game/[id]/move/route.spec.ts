...
const makeRequest = (gameId: string, body: object) =>
  new NextRequest(`http://localhost/api/game/${gameId}/move`, {
    method: 'POST',
...
describe('POST /api/game/[id]/move', async () => {
  const { POST } = await import('./route');

  it('returns 401 when no session', async () => {
    mockGetSession.mockResolvedValueOnce(null);

    const res = await POST(makeRequest('game-123', { position: 4 }), {
      params: makeParams('game-123'),
    });
    expect(res.status).toBe(401);
    expect(await res.json()).toEqual({ message: 'Unauthorized' });
  });

  it('returns updated board on success', async () => {
    mockGetSession.mockResolvedValueOnce(mockSession as never);
    mockApi.post.mockResolvedValueOnce({
      data: { board: Array(9).fill(null), result: null },
    });
    ...
    expect(mockApi.post).toHaveBeenCalledWith('/game/game-123/move', {
      position: 4,
    });
  });
  ...
  it('forwards axios error status', async () => {
    ...
    axiosError.response = { data: 'Invalid move', status: 400 } as never;
    ...
    expect(await res.json()).toEqual({ message: 'Invalid move' });
  });
